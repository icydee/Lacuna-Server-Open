...
=head2 get_star_chunk

This is a more efficient call to get star map details than get_star_map
since the data will be cached on the server for a short while. If you
(or someone in your alliance) requests the same data within a short period
then it will be retrieved from a cache file rather than the database.

It takes a hashref of named arguments.
...
Where each 'chunk' is a block of map which is 50 units wide by 50 units high.
So chunk (2,-3) covers the x co-ordinate from 100 - 149 and y co-ordinate from
-150 to -101

The area returned will typically contain about 60 stars with their planets.
